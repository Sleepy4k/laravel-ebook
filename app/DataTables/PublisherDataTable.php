...
class PublisherDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                return '<a href="'.route("table.publisher.show", $query->id).'"><i class="fa-solid fa-eye"></i></a> | <a href="'.route('table.publisher.edit', $query->id).'"><i class="fa-solid fa-pen-to-square"></i></a> | <form action="'.route("table.publisher.destroy", $query->id).'" method="post" style="display: inline-block">'.csrf_field().method_field("DELETE").'<a href="" class="confirm-delete"><i class="fa-solid fa-trash"></i></a></form>';
            })
            ->setRowId('id');
    }
    ...
    {
        return $this->builder()
                    ->setTableId('publisherdatatable-table')
